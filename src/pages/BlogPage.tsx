      ...
      <section className="py-16 lg:py-24">
        <div className="container">
          <div className="mb-12 flex flex-col items-center text-center">
            <div className="mb-5 inline-flex items-center rounded-[4px] bg-secondary px-4 py-1.5">
              <span className="text-sm font-medium tracking-[1px] text-white uppercase">Latest Articles</span>
            </div>
            <h2 className="font-neuliscursive text-[clamp(1.375rem,0.9806rem+1.6828vw,2.5rem)] font-bold text-primary">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {blogPosts.map((post, index) => (
              <AnimateOnScroll key={post.slug} animation="animate-fade-up" delay={index * 80} className="h-full">
                <Link
                  to={`/blog/${post.slug}`}
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6 lg:p-7">
                    ...
                    <p className="mb-2 font-inter text-xs font-medium text-text/50">{post.author}</p>
